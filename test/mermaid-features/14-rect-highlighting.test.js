...
//////////////////////////////////////////////////////////////////////////////
/**
 * Read a UTF-8 fixture file from the rect-highlighting feature slice.
 *
...
function readFixture(fileName) {
	return fs.readFileSync(path.join(__dirname, "14-rect-highlighting", fileName), "utf8");
}

//////////////////////////////////////////////////////////////////////////////
/**
 * Return the absolute path to a rect-highlighting fixture file.
 *
 * @param {string} fileName Fixture file name.
...
function getFixturePath(fileName) {
	return path.join(__dirname, "14-rect-highlighting", fileName);
}
...
describe("Mermaid feature slice 14: rect highlighting", () => {
	test("transforms Mermaid rect blocks into sequencer highlight fragments with actor spans", () => {
		const source = readFixture("input.mmd");
		...
		expect(transformed.lines[1].fragmentType).toBe("rect");
		expect(transformed.lines[1].bgColour).toBe("rgba(255, 220, 180, 0.5)");
		expect(transformed.lines[1].borderWidth).toBe(0);
		expect(transformed.lines[1].startActor).toBe("Service");
		expect(transformed.lines[1].endActor).toBe("DB");
		expect(transformed.lines[2].fragmentType).toBe("rect");
		expect(transformed.lines[2].borderWidth).toBe(0);
		expect(transformed.lines[2].startActor).toBe("Service");
		expect(transformed.lines[2].endActor).toBe("Cache");
	});

	test("renders SVG from Mermaid rect input and writes the transformed sequencer YAML sidecar", () => {
		const tempDir = createTempDir();
