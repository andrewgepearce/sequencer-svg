...
//////////////////////////////////////////////////////////////////////////////
/**
 * Read a UTF-8 fixture file from the actor-links feature slice.
 *
...
function readFixture(fileName) {
	return fs.readFileSync(path.join(__dirname, "21-actor-links", fileName), "utf8");
}

//////////////////////////////////////////////////////////////////////////////
/**
 * Return the absolute path to an actor-links fixture file.
 *
 * @param {string} fileName Fixture file name.
...
function getFixturePath(fileName) {
	return path.join(__dirname, "21-actor-links", fileName);
}
...
describe("Mermaid feature slice 21: actor links", () => {
	test("transforms Mermaid link statements into sequencer actor metadata", () => {
		const source = readFixture("input.mmd");
		...
		expect(transformed.actors[1].links).toEqual([
			{ label: "Schema", url: "https://docs.example.test/db" },
			{ label: "Runbook", url: "https://ops.example.test/db" },
		]);
	...
	test("renders SVG from Mermaid actor-link input and writes the transformed sequencer YAML sidecar", () => {
		const tempDir = createTempDir();
